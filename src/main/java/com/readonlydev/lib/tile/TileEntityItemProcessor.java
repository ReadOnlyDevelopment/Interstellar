...
package com.readonlydev.lib.tile;
...
import com.readonlydev.api.tile.IFuelConsumer;
import com.readonlydev.lib.annotation.SyncVariable;
import com.readonlydev.lib.math.MathUtil;
import com.readonlydev.lib.utils.InventoryUtil;
...
public abstract class TileEntityItemProcessor extends TileSidedInventoryExo implements ITickable {
	...
	/**
	 * Output overflow, because verifying whether or not items can fit in the output slots could be expensive, since multiple items could come from a single operation. This variable is reassigned whenever all outputs cannot fit.
	 */
	private Collection<ItemStack>	overflow	= ImmutableList.of();
	private int						timer		= MathUtil.nextInt(LONG_UPDATE_DELAY);
	...
	public TileEntityItemProcessor(int inputCount, int fuelCount, int outputCount, @Nullable IFuelConsumer burner) {
		if (inputCount < 1) {
	...
	protected boolean doWork() {
		...
		if (!inputs.isEmpty() && canWork(inputs) && hasSpaceForOutputs() && (burner == null || burner.hasFuel())) {
			...
			if (progress >= workTime) {
				...
				result = InventoryUtil.mergeItems(this, slotStart, slotStart + getOutputSlotCount(), result);
				if (!result.isEmpty()) {
					overflow = result;
				}
	...
	public void update() {
		...
		if (!world.isRemote) {
			...
			if (timer % LONG_UPDATE_DELAY == 0 && !overflow.isEmpty()) {
				...
				overflow = InventoryUtil.mergeItems(this, start, start + getOutputSlotCount(), overflow);
