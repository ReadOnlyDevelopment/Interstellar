...
public final class ItemStackList extends ArrayList<ItemStack> {
	private static final long serialVersionUID = 2130196648481523228L;
	 ...
	 */
	public static ItemStackList fromInventory(IInventory inventory) {
		ItemStackList newList = new ItemStackList();
	...
	public static ItemStackList fromNBT(NBTTagList tagList) {
		ItemStackList newList = new ItemStackList();
	...
	public ItemStack uniqueMatch(Predicate<ItemStack> predicate) {
		return stream().filter(predicate).collect(Collectors.collectingAndThen(Collectors.toList(), list -> list.size() == 1 ? list.get(0) : ItemStack.EMPTY));
	}
