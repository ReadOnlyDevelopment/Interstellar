...
import com.readonlydev.lib.world.gen.chunk.ChunkLandscape;
import com.readonlydev.lib.world.noise.CircularSearchCreator;
...
public final class BiomeAnalyzer {
	...
	private void setupBeachesForBiomes() {
		...
		for (int i = 0; i < preferredBeach.length; i++) {
			...
			// We need to work with the realistic biome, so let's try to get it from the
			// base biome, aborting if necessary.
	...
	public void newRepair(final Biome[] genLayerBiomes, final int[] biomeNeighborhood, final ChunkLandscape landscape) {
		...
		for (int i = 0; i < MAX_BIOMES; i++) {
			...
			// skip if this block isn't above beach level, adjusted for river effect to
			// prevent abrupt beach stops
	...
	private static final class SmoothingSearchStatus {
		...
		// weightings is part of a system to generate some variability in repaired
		// chunks weighting is
		// based on how long the search went on (so quasipsuedorandom, based on
		...
		private void hunt(int[] biomeNeighborhood) {
			// 0,0 in the chunk is 9,9 int the array ; 8,8 is 10,10 and is treated as the
			// center
			clear();
		...
		private void smoothQuadrant(int biomesOffset, int findingsOffset) {
			...
			for (int x = 0; x < 8; x++) {
				for (int z = 0; z < 8; z++) {
					...
					// they go to 0 on the far edges so only the points on the edge have effects
					// there
