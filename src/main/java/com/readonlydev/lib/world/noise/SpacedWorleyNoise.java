...
public class SpacedWorleyNoise implements WorleyNoise {
	...
	private Point2D.Double[] generatedAreaPoints(Point area) {
		...
		for (int i = 0; i < pointsPerTorus; i++) {
			...
			result[i] = new Point2D.Double(this.allPoints[index].getX() + area.getX(),
					this.allPoints[index].getY() + area.getY());
			used[i] = true;
		}
