...
//		}
//		float lakeStrength = lakePressure(exoWorld, x, y, border, exoWorld.getLakeFrequency(), exoWorld.getLakeBendSizeLarge(), exoWorld.getLakeBendSizeMedium(), exoWorld.getLakeBendSizeSmall());
//		float lakeFlattening = lakeFlattening(lakeStrength, exoWorld.getLakeShoreLevel(), exoWorld.getLakeDepressionLevel());
//
...
//		river = 1f - river;
//
//		float riverFlattening = river * (1f + ExoplanetWorld.RIVER_FLATTENING_ADDEND) - ExoplanetWorld.RIVER_FLATTENING_ADDEND;
...
//			float irregularity = exoWorld.simplexInstance(0).noise2f(x / 12f, y / 12f) * 2f + exoWorld.simplexInstance(0).noise2f(x / 8f, y / 8f);
...
//
//	@Override
//	public float lakePressure(ExoplanetWorld exoWorld, int x, int y, float border, float lakeInterval, float largeBendSize, float mediumBendSize, float smallBendSize) {
...
//
//	@Override
//	public void rReplace(ChunkPrimer primer, BlockPos blockPos, int x, int y, int depth, ExoplanetWorld exoWorld, float[] noise, float river, Biome[] base) {
//		rReplace(primer, blockPos.getX(), blockPos.getZ(), x, y, depth, exoWorld, noise, river, base);
//	}
//
//	@Override
//	public void rReplace(ChunkPrimer primer, int i, int j, int x, int y, int depth, ExoplanetWorld exoWorld, float[] noise, float river, Biome[] base) {
...
//	}
//
//	protected void rReplaceWithRiver(ChunkPrimer primer, int i, int j, int x, int y, int depth, ExoplanetWorld exoWorld, float[] noise, float river, Biome[] base) {
//		float riverRegion = !this.allow_rivers ? 0f : river;
//		this.surface.paintTerrain(primer, i, j, x, y, depth, exoWorld, noise, riverRegion, base);
//	}
//
...
//		if (getBiome().getDefaultTemperature() <= 0.05f || BiomeDictionary.hasType(getBiome(), BiomeDictionary.Type.SNOWY)) {
