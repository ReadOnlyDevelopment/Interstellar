 ...
 */
package com.readonlydev.lib.world.gen.cave;

import com.google.common.base.MoreObjects;
import com.readonlydev.lib.world.gen.CaveGenUtility;
import com.readonlydev.lib.world.gen.FastNoise;
...
public class CaveGeneratorExoplanet extends MapGenBase {
						...
						for (int subx = 0; subx < 4; subx++) {
							...
							for (int subz = 0; subz < 4; subz++) {
								...
								if (noiseVal > adjustedNoiseCutoff) {
									IBlockState aboveBlock = MoreObjects.firstNonNull(
	...
	private int getMaxSurfaceHeight(ChunkPrimer primer) {
		...
		for (int[] testcord : testcords) {
			...
			int testmax = getSurfaceHeight(primer, testcord[0], testcord[1]);
			if (testmax > max) {
				max = testmax;
