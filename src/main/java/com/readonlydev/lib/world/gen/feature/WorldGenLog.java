package com.readonlydev.lib.world.gen.feature;
...
public class WorldGenLog extends WorldGenerator {
	...
	public boolean generate(@Nonnull World world, @Nonnull Random rand, @Nonnull BlockPos pos) {
		...
		if (g.getMaterial() != Material.GROUND && g.getMaterial() != Material.GRASS && g.getMaterial() != Material.SAND
		...
		ArrayList<IBlockState> aBlock = new ArrayList<>();
		...
		for (int i = 0; i < logLength * 2; i++) {
			...
			/*
			 * Before we place the log block, let's make sure that there's an air block
			 * immediately above it. This is to ensure that the log doesn't override, for
			 * example, a 2-block tall plant, which some mods (like WAILA) have trouble
			 * handling.
			 *
			 * Also, to ensure that we don't have 'broken' logs, if one log block fails the
			 * check, then no logs actually get placed.
			 */
			...
			// If we can't rotate the log block for whatever reason, then don't even try
			// placing it and bail.
			// We'd rather generate nothing than something ugly.
			try {
				aBlock.add(logBlock.withProperty(BlockLog.LOG_AXIS,
	...
	private int airCheck(World world, int x, int y, int z) {
		...
		if (b.getMaterial() == Material.AIR || b.getMaterial() == Material.VINE || b.getMaterial() == Material.WATER
				|| b.getMaterial() == Material.PLANTS) {
