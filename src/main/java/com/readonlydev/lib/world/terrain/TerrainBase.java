package com.readonlydev.lib.world.terrain;
...
import com.readonlydev.lib.world.terrain.heighteffect.VariableRuggednessEffect;
...
public abstract class TerrainBase {
	...
	private static final float minimumDuneHeight = 21f; // The strength factor to which the dune height config option is
														// added.
	...
	public static float blendedHillHeight(float simplex, float turnAt) {
		// like blendedHillHeight, but the effect of zero occurs at the turnAt parameter
		// instead
	...
	public static float hills(float x, float y, float hillStrength, ExoplanetWorld exoWorld) {
		...
		float sm = exoWorld.simplexInstance(2).noise2f(x / 55, y / 55);// there are artifacts if this is close to a
																		// multiple of 16
	...
	public static float terrainCanyon(int x, int y, ExoplanetWorld exoWorld, float river, float[] height, float border,
	...
	public static float terrainFlatLakes(int x, int y, ExoplanetWorld exoWorld, float river, float baseHeight) {
		/*
		 * float h = simplex.noise2f(x / 300f, y / 300f) * 40f * river; h = h > hMax ?
		 * hMax : h; h += simplex.noise2f(x / 50f, y / 50f) * (12f - h) * 0.4f; h +=
		 * simplex.noise2f(x / 15f, y / 15f) * (12f - h) * 0.15f;
		 */

		float ruggedNoise = exoWorld.simplexInstance(1).noise2f(
		...
		float h = groundNoise(x, y, 2f * (ruggedNoise + 1f), exoWorld);// ground noise
	...
	public static float terrainGrasslandFlats(int x, int y, ExoplanetWorld exoWorld, float river, float mPitch,
			float baseHeight) {
	...
	public static float terrainGrasslandHills(int x, int y, ExoplanetWorld exoWorld, float river, float vWidth,
			float vHeight, float hWidth, float hHeight, float bHeight) {
	...
	public static float terrainGrasslandMountains(int x, int y, ExoplanetWorld exoWorld, float river, float hFactor,
			float mFactor, float baseHeight) {
	...
	public static float terrainHighland(float x, float y, ExoplanetWorld exoWorld, float river, float start,
			float width, float height, float baseAdjust) {

		float h = exoWorld.simplexInstance(0).noise2f(x / width, y / width) * height * river; // -140 to 140
		...
		if (h > 0f) {
			...
			h += exoWorld.simplexInstance(4).noise3f(x / 70f, y / 70f, 1f) * st;// 0 to 155
			h = h * river;
		}
	...
	public static float terrainLonelyMountain(int x, int y, ExoplanetWorld exoWorld, float river, float strength,
			float width, float terrainHeight) {
		...
		// the parameters can "blow through the ceiling" so pull more extreme values
		// down a bit
	...
	public static float terrainOceanCanyon(int x, int y, ExoplanetWorld exoWorld, float river, float[] height,
	...
	public static float terrainPlains(int x, int y, ExoplanetWorld exoWorld, float river, float stPitch, float stFactor,
			float hPitch, float hDivisor, float baseHeight) {
	...
	public static float terrainPlateau(float x, float y, ExoplanetWorld exoWorld, float river, float[] height,
	...
	public static float terrainPolar(float x, float y, ExoplanetWorld exoWorld, float river, float stPitch,
	...
	public static float terrainRollingHills(int x, int y, ExoplanetWorld exoWorld, float river, float hillStrength,
			float addedHeight, float groundNoiseAmplitudeHills, float lift) {
	...
	public static float terrainRollingHills(int x, int y, ExoplanetWorld exoWorld, float river, float hillStrength,
			float groundNoiseAmplitudeHills, float baseHeight) {
	...
	public static float terrainVolcano(int x, int y, ExoplanetWorld exoWorld, float border, float baseHeight) {
		...
		float st = 15f - (float) (worleyNoise.eval2D(x / 500d, y / 500d).getShortestDistance() * 42d)
		...
		if (h > 10f) {
			...
			h += worleyNoise.eval2D(x / 25D, y / 25D).getShortestDistance() * d2;
		}
	...
	public static float getRiverStrength(final BlockPos blockPos, final ExoplanetWorld exoWorld) {
		...
		// New worley noise.
		double riverFactor = exoWorld.worleyInstance(0).eval2D(pX, pZ).interiorValue();

		// the output is a curved function of relative distance from the center, so
		// adjust to make it flatter
		riverFactor = bayesianAdjustment((float) riverFactor, 0.5f);
		if (riverFactor > exoWorld.getRiverValleyLevel()) {
			return 0;
		} // no river effect
		return (float) (riverFactor / exoWorld.getRiverValleyLevel() - 1d);
	}
	...
	public static void calcSnowHeight(int x, int y, int z, ChunkPrimer primer, float[] noise) {
		if (y < 254) {
			...
			if (h > 7) {
				...
				primer.setBlockState(x, y + 1, z,
						Blocks.SNOW_LAYER.getDefaultState().withProperty(BlockSnow.LAYERS, 7));
			} else if (h > 0) {
				primer.setBlockState(x, y + 1, z,
						Blocks.SNOW_LAYER.getDefaultState().withProperty(BlockSnow.LAYERS, (int) h));
			}
		}
	}

	public static float bayesianAdjustment(float probability, float multiplier) {
		// returns the original probability adjusted for the multiplier to the
		// confidence ratio
