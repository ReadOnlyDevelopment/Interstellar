...
package com.readonlydev.lib.recipe;
...
import com.readonlydev.api.item.IItemEnum;
import com.readonlydev.api.repcie.IObjectSerialize;
...
public final class RecipeBuilder {
	...
	private final Map<ResourceLocation, IObjectSerialize>	customSerializers	= new HashMap<>();
	 ...
	 */
	public void setRecipeSerializer(Item item, IObjectSerialize serializer) {
		customSerializers.put(item.getRegistryName(), serializer);
	}

	/*
	 * Recipes adders and makers. Adders will make and register a recipe. Makers just create a recipe (useful for guide book stuff, etc.) Group will default to the mod ID if not specified.
	 */
	...
	/**
	 * Adds a compression recipe. For example, crafting ingots into blocks and vice versa. This will add both the compression (small to big) and decompression (big to small) recipes.
	 *
	 ...
	 * @return Both created recipes in an array. First is compression, second is decompression. They are both ShapelessRecipes.
	 */
	...
	private ItemStack[] makeStackArray(Object... params) {
		...
		for (int i = 0; i < params.length; ++i) {
			...
			} else if (obj instanceof IItemEnum) {
				result[i] = ((IItemEnum<?, ?>) obj).getStack();
			} else {
