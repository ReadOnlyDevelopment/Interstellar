...
import net.interstellar.lib.asm.ASMUtil;
...
public class EntityUtil {
	...
	static {
		try {
			...
			InterstellarLogger.error("Failed to get PlayerChunkMap class.", e);
		}
...
//				return player;
	 ...
	 * If <b>sixWays</b> is <code>true</code>, the direction can be {@link EnumFacing#UP UP} or {@link EnumFacing#DOWN DOWN} if the entity is looking up or down.
	 *
	 * @param entity  the entity
	...
	public static List<EntityPlayerMP> getPlayersWatchingChunk(WorldServer world, int x, int z) {
		...
		try {
			...
			InterstellarLogger.info("Failed to get players watching chunk :", e);
			return new ArrayList<>();
		}
	}
		...
		for (int i = 0; i < nb; ++i) {
			for (int j = 0; j < nb; ++j) {
				for (int k = 0; k < nb; ++k) {
					...
					ParticleDigging fx = (ParticleDigging) factory.createParticle(0, world, fxX, fxY, fxZ, fxX - pos.getX() - 0.5D, fxY - pos.getY() - 0.5D, fxZ - pos.getZ() - 0.5D, id);
