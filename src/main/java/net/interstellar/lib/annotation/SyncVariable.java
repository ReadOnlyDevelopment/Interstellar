...
package net.interstellar.lib.annotation;
...
import net.interstellar.api.nbt.INBTData;
...
public @interface SyncVariable {
	...
	final class Helper {
		...
		static final Map<Class, INBTData>						SERIALIZERS	= new HashMap<>();

		private Helper() {
		}

		public static <T> void registerSerializer(Class<T> clazz, Function<NBTTagCompound, T> reader, BiConsumer<NBTTagCompound, T> writer) {
			SERIALIZERS.put(clazz, new INBTData<T>() {
				@Override
		...
		/**
		 * Reads sync variables for the object. This method will attempt to read a value from NBT and assign that value for any field marked with the SyncVariable annotation.
		 *
					...
					if (annotation instanceof SyncVariable) {
						...
						try {
							...
							if (field.getType() == int.class) {
								...
								INBTData serializer = SERIALIZERS.get(field.getType());
		...
		/**
		 * Writes sync variables for the object. This method will take the values in all fields marked with the SyncVariable annotation and save them to NBT.
		 *
		 * @param obj      The object with SyncVariable fields.
						...
						if (syncType == SyncVariable.Type.WRITE && sync.onWrite() || syncType == SyncVariable.Type.PACKET && sync.onPacket()) {
							try {
								...
								if (field.getType() == int.class) {
									...
									INBTData<Object> serializer = SERIALIZERS.get(field.getType());
