 ...
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */
package net.interstellar.lib.recipe.ingredient;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

import net.interstellar.lib.utils.OreUtil;
...
public class IngredientStack {
	...
	public boolean matches(Object input) {
		...
		if (input instanceof ItemStack) {
			...
			return OreUtil.compareToOreName(stack, (String) input);
		}
		return false;
	}
	...
	public List<ItemStack> getStackList() {
		...
		if ((fluid != null) && (ForgeModContainer.getInstance().universalBucket != null)) {
			return Collections.singletonList(FluidUtil.getFilledBucket(fluid));
		}
	...
	public List<ItemStack> getSizedStackList() {
		...
		if (oreName != null) {
			...
			for (ItemStack stack : OreDictionary.getOres(oreName)) {
				list.add(OreUtil.copyStackWithAmount(stack, inputSize));
			}
		} else if ((fluid != null) && (ForgeModContainer.getInstance().universalBucket != null)) {
			list = Collections.singletonList(FluidUtil.getFilledBucket(fluid));
		} else if (stackList != null) {
			list = stackList;
		} else {
			list = Collections.singletonList(OreUtil.copyStackWithAmount(stack, inputSize));
		}
		return list;
	}
	...
	public ItemStack getRandomizedExampleStack(long rand) {
		...
		if (ret.isEmpty() && (fluid != null) && (ForgeModContainer.getInstance().universalBucket != null)) {
			ret = FluidUtil.getFilledBucket(fluid);
		}
		return ret;
	}
	...
	public Ingredient toRecipeIngredient() {
		...
		if ((ret == null) && (stackList != null) && (stackList.size() > 0)) {
			ret = OreUtil.createIngredientFromList(stackList);
		}
	...
	public boolean matchesItemStack(ItemStack input) {
		...
		if (this.oreName != null) {
			return OreUtil.compareToOreName(input, oreName) && (this.inputSize <= input.getCount());
		}
	...
	public boolean matchesItemStackIgnoringSize(ItemStack input) {
		...
		if (this.oreName != null) {
			return OreUtil.compareToOreName(input, oreName);
		}
		if (this.stackList != null) {
			for (ItemStack iStack : this.stackList) {
